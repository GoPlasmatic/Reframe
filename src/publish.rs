...
impl AsyncFunctionHandler for PublishFunction {
    async fn execute(&self, message: &mut Message, input: &Value) -> Result<(usize, Vec<Change>)> {
        ...
        let data = message.data.get(input_field_name)
            .ok_or_else(|| DataflowError::Validation(format!("Field {} not found in message data", input_field_name)))?;
        ...
        if output_format == "pacs.008.001.13" {
            ...
            if let Some(fi_to_fi) = data.get("FIToFICstmrCdtTrf") {
                match serde_json::from_value::<mx_message::pacs_008_001_13::FIToFICustomerCreditTransferV13>(fi_to_fi.clone()) {
                    Ok(pacs_data) => {
                        // Create the Document enum
                        let document = mx_message::document::Document::FIToFICustomerCreditTransferV13(Box::new(pacs_data));
                        ...
                        match xml_to_string(&document) {
                            ...
                            Err(e) => {
                                ...
                                return Err(DataflowError::Validation(format!("XML serialization failed: {}", e)));
                            }
                        }
                    },
                    Err(e) => {
                        println!("FIToFICustomerCreditTransferV13 deserialization failed: {}", e);
                        return Err(DataflowError::Validation(format!("FIToFICustomerCreditTransferV13 deserialization failed: {}", e)));
                    }
                }
            } else {
                return Err(DataflowError::Validation(format!("FIToFICustomerCreditTransferV13 not found in data")));
            }
        } else {
            Err(DataflowError::Validation(format!("Unsupported format: {}", output_format)))
        }
    }
}