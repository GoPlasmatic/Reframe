...
fn generate_camt058_xml(app_hdr: &Value, document: &Value) -> Result<String, Box<dyn std::error::Error>> {
    debug!("Parsing camt.058 header");
    let header_str = serde_json::to_string(app_hdr)?;
    let hd = &mut serde_json::Deserializer::from_str(&header_str);
    let header: bah_camt_058_001_08::BusinessApplicationHeaderV02 = deserialize(hd)
        .map_err(|e| {
            let path = e.path().to_string();
            let inner_err = e.into_inner().to_string();
            format!("Failed to parse camt.058 header at path '{}': {}", path, inner_err)
        })?;
    
    debug!("Parsing camt.058 document");
    let doc_content = document
        .get("NtfctnToRcvCxlAdvc")
        .ok_or_else(|| "Missing NtfctnToRcvCxlAdvc in Document")?;
    ...
    let doc: camt_058_001_08::NotificationToReceiveCancellationAdviceV08 = deserialize(dd)
        .map_err(|e| {
            let path = e.path().to_string();
            let inner_err = e.into_inner().to_string();
            error!("Failed to parse camt.058 document at path '{}': {}", path, inner_err);
            format!("Failed to parse camt.058 document at path '{}': {}", path, inner_err)
        })?;
    
    let envelope = MxEnvelope::new(header, doc, "urn:iso:std:iso:20022:tech:xsd:camt.058.001.08".to_string());
    let xml_declaration = r#"<?xml version="1.0" encoding="UTF-8"?>"#;
    let xml_body = xml_to_string(&envelope)?;
...
fn generate_camt105_xml(app_hdr: &Value, document: &Value) -> Result<String, Box<dyn std::error::Error>> {
    debug!("Parsing camt.105 header");
    let header_str = serde_json::to_string(app_hdr)?;
    let hd = &mut serde_json::Deserializer::from_str(&header_str);
    let header: bah_camt_105_001_02::BusinessApplicationHeaderV02 = deserialize(hd)
        .map_err(|e| {
            let path = e.path().to_string();
            let inner_err = e.into_inner().to_string();
            format!("Failed to parse camt.105 header at path '{}': {}", path, inner_err)
        })?;
    ...
    let doc_content = document
        ...
        .ok_or_else(|| "Missing ChrgsPaymtNtfctn in Document")?;
    
    debug!("camt.105 doc_content: {}", serde_json::to_string_pretty(doc_content)?);
    let doc_str = serde_json::to_string(doc_content)?;
    let dd = &mut serde_json::Deserializer::from_str(&doc_str);
    let doc: camt_105_001_02::ChargesPaymentNotificationV02 = deserialize(dd)
        .map_err(|e| {
            let path = e.path().to_string();
            let inner_err = e.into_inner().to_string();
            error!("Failed to parse camt.105 document at path '{}': {}", path, inner_err);
            format!("Failed to parse camt.105 document at path '{}': {}", path, inner_err)
        })?;
    
    let envelope = MxEnvelope::new(header, doc, "urn:iso:std:iso:20022:tech:xsd:camt.105.001.02".to_string());
    let xml_declaration = r#"<?xml version="1.0" encoding="UTF-8"?>"#;
    let xml_body = xml_to_string(&envelope)?;
