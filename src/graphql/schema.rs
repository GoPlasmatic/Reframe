...
fn build_query_type() -> Object {
    Object::new("Query")
        ...
        .field(Field::new(
            "message",
            TypeRef::named("TransformationMessage"),
            |ctx| FieldFuture::new(async move {
                ...
                Ok(msg.map(|m| FieldValue::owned_any(m)))
            })
        ).argument(dynamic::InputValue::new("id", TypeRef::named_nn(TypeRef::STRING))))

        // messages(filter, limit, offset, recompute_custom_fields): MessageConnection
        .field(Field::new(
            "messages",
            TypeRef::named_nn("MessageConnection"),
            |ctx| FieldFuture::new(async move {
                let filter = ctx.args.try_get("filter")
                    .ok()
                    .and_then(|v| serde_json::from_value::<MessageFilter>(v.deserialize().ok()?).ok());
                let limit = ctx.args.try_get("limit")
                    .and_then(|v| v.i64())
                    .unwrap_or(50)
                    .min(1000);
                let offset = ctx.args.try_get("offset")
                    .and_then(|v| v.i64())
                    .unwrap_or(0);
                let recompute = ctx.args.try_get("recomputeCustomFields")
        ...
        .argument(dynamic::InputValue::new("filter", TypeRef::named("MessageFilter")))
        .argument(dynamic::InputValue::new("limit", TypeRef::named(TypeRef::INT)).default_value(50))
        .argument(dynamic::InputValue::new("offset", TypeRef::named(TypeRef::INT)).default_value(0))
        .argument(dynamic::InputValue::new("recomputeCustomFields", TypeRef::named(TypeRef::BOOLEAN)).default_value(false)))
        ...
        .field(Field::new(
            ...
            |ctx| FieldFuture::new(async move {
                ...
                let result: Vec<FieldValue> = messages
                    .into_iter()
                    .map(|m| FieldValue::owned_any(m))
                    .collect();

                Ok(Some(FieldValue::list(result)))
            })
        )
        .argument(dynamic::InputValue::new("query", TypeRef::named_nn(TypeRef::STRING)))
        .argument(dynamic::InputValue::new("limit", TypeRef::named(TypeRef::INT)).default_value(50)))
...
pub fn create_schema(
    ...
    let message_type = dynamic_schema::build_transformation_message_type(
        pm.get_packages()
    );
    ...
    schema_builder.finish().expect("Failed to build GraphQL schema")
}
