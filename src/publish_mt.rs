...
use swift_mt_message::messages::{
    MT103, MT110, MT111, MT112, MT190, MT192, MT196, MT199, MT202, MT205, MT210, MT290, MT292, MT296, MT299, MT940, MT942,
};
        ...
        let mt_message = if source_format == "pacs.008.001.08" {
            ...
            if mt_type == "192" {
                let data: SwiftMessage<MT192> = serde_json::from_str(&json_str).map_err(|e| {
                    ...
                    DataflowError::Validation(format!("Failed to parse JSON string for camt.056 (MT192): {e}"))
                })?;
                data.to_mt_message()
            } else {
                let data: SwiftMessage<MT292> = serde_json::from_str(&json_str).map_err(|e| {
                    ...
                    DataflowError::Validation(format!("Failed to parse JSON string for camt.056 (MT292): {e}"))
                })?;
                data.to_mt_message()
            }
            ...
            let mt_type = cleaned_data.get("message_type")
                .and_then(|v| v.as_str())
                .unwrap_or("190");
            ...
            if mt_type == "190" {
                let data: SwiftMessage<MT190> = serde_json::from_str(&json_str).map_err(|e| {
                    ...
                    DataflowError::Validation(format!("Failed to parse JSON string for camt.105 (MT190): {e}"))
                })?;
                data.to_mt_message()
            } else {
                let data: SwiftMessage<MT290> = serde_json::from_str(&json_str).map_err(|e| {
                    ...
                    DataflowError::Validation(format!("Failed to parse JSON string for camt.105 (MT290): {e}"))
                })?;
                data.to_mt_message()
            }
