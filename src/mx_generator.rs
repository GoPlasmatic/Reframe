...
fn generate_pacs008_xml(
    ...
    let header: bah_pacs_008_001_08::BusinessApplicationHeaderV02 =
        deserialize(hd).map_err(|e| {
            ...
            format!(
                "Failed to parse pacs.008 header at path '{}': {}",
                path, inner_err
            )
        })?;
    ...
    let doc: pacs_008_001_08::FIToFICustomerCreditTransferV08 = deserialize(dd).map_err(|e| {
        let path = e.path().to_string();
        let inner_err = e.into_inner().to_string();
        error!(
...
fn generate_pacs002_xml(
    ...
    let header: bah_pacs_002_001_10::BusinessApplicationHeaderV02 =
        deserialize(hd).map_err(|e| {
            ...
            let inner_err = e.into_inner().to_string();
            format!(
                "Failed to parse pacs.002 header at path '{}': {}",
                path, inner_err
            )
        })?;
    ...
    let doc_str = serde_json::to_string(doc_content)?;
    let dd = &mut serde_json::Deserializer::from_str(&doc_str);
    let doc: pacs_002_001_10::FIToFIPaymentStatusReportV10 = deserialize(dd).map_err(|e| {
        let path = e.path().to_string();
        let inner_err = e.into_inner().to_string();
        error!(
            "Failed to parse pacs.002 document at path '{}': {}",
            path, inner_err
        );
        format!(
            "Failed to parse pacs.002 document at path '{}': {}",
    ...
    let envelope = MxEnvelope::new(
        header,
        doc,
        "urn:iso:std:iso:20022:tech:xsd:pacs.002.001.10".to_string(),
    );
    let xml_declaration = r#"<?xml version="1.0" encoding="UTF-8"?>"#;
    let xml_body = xml_to_string(&envelope)?;
...
fn generate_pacs004_xml(
    ...
    let header: bah_pacs_004_001_09::BusinessApplicationHeaderV02 =
        deserialize(hd).map_err(|e| {
            ...
            let inner_err = e.into_inner().to_string();
            format!(
                "Failed to parse pacs.004 header at path '{}': {}",
                path, inner_err
            )
        })?;
    ...
    let doc_str = serde_json::to_string(doc_content)?;
    let dd = &mut serde_json::Deserializer::from_str(&doc_str);
    let doc: pacs_004_001_09::PaymentReturnV09 = deserialize(dd).map_err(|e| {
        let path = e.path().to_string();
        let inner_err = e.into_inner().to_string();
        error!(
            "Failed to parse pacs.004 document at path '{}': {}",
            path, inner_err
        );
        format!(
            "Failed to parse pacs.004 document at path '{}': {}",
    ...
    let envelope = MxEnvelope::new(
        header,
        doc,
        "urn:iso:std:iso:20022:tech:xsd:pacs.004.001.09".to_string(),
    );
    let xml_declaration = r#"<?xml version="1.0" encoding="UTF-8"?>"#;
    let xml_body = xml_to_string(&envelope)?;
...
fn generate_pacs009_xml(
    ...
    let header: bah_pacs_009_001_08::BusinessApplicationHeaderV02 =
        deserialize(hd).map_err(|e| {
            ...
            let inner_err = e.into_inner().to_string();
            format!(
                "Failed to parse pacs.009 header at path '{}': {}",
                path, inner_err
            )
        })?;
    ...
    let doc: pacs_009_001_08::FinancialInstitutionCreditTransferV08 =
        deserialize(dd).map_err(|e| {
            ...
            let inner_err = e.into_inner().to_string();
            error!(
                "Failed to parse pacs.009 document at path '{}': {}",
                path, inner_err
            );
            format!(
                "Failed to parse pacs.009 document at path '{}': {}",
...
fn generate_camt052_xml(
    ...
    let header: bah_camt_052_001_08::BusinessApplicationHeaderV02 =
        deserialize(hd).map_err(|e| {
            ...
            let inner_err = e.into_inner().to_string();
            format!(
                "Failed to parse camt.052 header at path '{}': {}",
                path, inner_err
            )
        })?;
    ...
    let doc_str = serde_json::to_string(doc_content)?;
    let dd = &mut serde_json::Deserializer::from_str(&doc_str);
    let doc: camt_052_001_08::BankToCustomerAccountReportV08 = deserialize(dd).map_err(|e| {
        let path = e.path().to_string();
        let inner_err = e.into_inner().to_string();
        error!(
            "Failed to parse camt.052 document at path '{}': {}",
            path, inner_err
        );
        format!(
            "Failed to parse camt.052 document at path '{}': {}",
...
fn generate_camt053_xml(
    ...
    let header: bah_camt_053_001_08::BusinessApplicationHeaderV02 =
        deserialize(hd).map_err(|e| {
            ...
            let inner_err = e.into_inner().to_string();
            format!(
                "Failed to parse camt.053 header at path '{}': {}",
                path, inner_err
            )
        })?;
    ...
    let doc_str = serde_json::to_string(doc_content)?;
    let dd = &mut serde_json::Deserializer::from_str(&doc_str);
    let doc: camt_053_001_08::BankToCustomerStatementV08 = deserialize(dd).map_err(|e| {
        let path = e.path().to_string();
        let inner_err = e.into_inner().to_string();
        error!(
            "Failed to parse camt.053 document at path '{}': {}",
            path, inner_err
        );
        format!(
            "Failed to parse camt.053 document at path '{}': {}",
...
fn generate_camt107_xml(
    ...
    let header: bah_camt_107_001_01::BusinessApplicationHeaderV02 =
        deserialize(hd).map_err(|e| {
            ...
            let inner_err = e.into_inner().to_string();
            format!(
                "Failed to parse camt.107 header at path '{}': {}",
                path, inner_err
            )
        })?;
    ...
    let doc_str = serde_json::to_string(doc_content)?;
    let dd = &mut serde_json::Deserializer::from_str(&doc_str);
    let doc: camt_107_001_01::ChequePresentmentNotificationV01 = deserialize(dd).map_err(|e| {
        let path = e.path().to_string();
        let inner_err = e.into_inner().to_string();
        error!(
            "Failed to parse camt.107 document at path '{}': {}",
            path, inner_err
        );
        format!(
            "Failed to parse camt.107 document at path '{}': {}",
...
fn generate_camt108_xml(
    ...
    let header: bah_camt_108_001_01::BusinessApplicationHeaderV02 =
        deserialize(hd).map_err(|e| {
            ...
            let inner_err = e.into_inner().to_string();
            format!(
                "Failed to parse camt.108 header at path '{}': {}",
                path, inner_err
            )
        })?;
    ...
    let doc: camt_108_001_01::ChequeCancellationOrStopRequestV01 =
        deserialize(dd).map_err(|e| {
            ...
            let inner_err = e.into_inner().to_string();
            error!(
                "Failed to parse camt.108 document at path '{}': {}",
                path, inner_err
            );
            format!(
                "Failed to parse camt.108 document at path '{}': {}",
...
fn generate_camt109_xml(
    ...
    let header: bah_camt_109_001_01::BusinessApplicationHeaderV02 =
        deserialize(hd).map_err(|e| {
            ...
            let inner_err = e.into_inner().to_string();
            format!(
                "Failed to parse camt.109 header at path '{}': {}",
                path, inner_err
            )
        })?;
    ...
    let doc_str = serde_json::to_string(doc_content)?;
    let dd = &mut serde_json::Deserializer::from_str(&doc_str);
    let doc: camt_109_001_01::ChequeCancellationOrStopReportV01 = deserialize(dd).map_err(|e| {
        let path = e.path().to_string();
        let inner_err = e.into_inner().to_string();
        error!(
            "Failed to parse camt.109 document at path '{}': {}",
            path, inner_err
        );
        format!(
            "Failed to parse camt.109 document at path '{}': {}",
...
fn generate_camt054_xml(
    ...
    let header: bah_camt_052_001_08::BusinessApplicationHeaderV02 =
        deserialize(hd).map_err(|e| {
            ...
            let inner_err = e.into_inner().to_string();
            format!(
                "Failed to parse camt.054 header at path '{}': {}",
                path, inner_err
            )
        })?;
    ...
    let doc_str = serde_json::to_string(doc_content)?;
    let dd = &mut serde_json::Deserializer::from_str(&doc_str);
    let doc: camt_054_001_08::BankToCustomerDebitCreditNotificationV08 =
        deserialize(dd).map_err(|e| {
            let path = e.path().to_string();
            let inner_err = e.into_inner().to_string();
            error!(
                "Failed to parse camt.054 document at path '{}': {}",
                path, inner_err
            );
            format!(
                "Failed to parse camt.054 document at path '{}': {}",
    ...
    let envelope = MxEnvelope::new(
        header,
        doc,
        "urn:iso:std:iso:20022:tech:xsd:camt.054.001.08".to_string(),
    );
    let xml_declaration = r#"<?xml version="1.0" encoding="UTF-8"?>"#;
    let xml_body = xml_to_string(&envelope)?;
...
fn generate_camt056_xml(
    ...
    let header: bah_camt_056_001_08::BusinessApplicationHeaderV02 =
        deserialize(hd).map_err(|e| {
            ...
            let inner_err = e.into_inner().to_string();
            format!(
                "Failed to parse camt.056 header at path '{}': {}",
                path, inner_err
            )
        })?;
    ...
    let doc_str = serde_json::to_string(doc_content)?;
    let dd = &mut serde_json::Deserializer::from_str(&doc_str);
    let doc: camt_056_001_08::FIToFIPaymentCancellationRequestV08 =
        deserialize(dd).map_err(|e| {
            let path = e.path().to_string();
            let inner_err = e.into_inner().to_string();
            error!(
                "Failed to parse camt.056 document at path '{}': {}",
                path, inner_err
            );
            format!(
                "Failed to parse camt.056 document at path '{}': {}",
    ...
    let envelope = MxEnvelope::new(
        header,
        doc,
        "urn:iso:std:iso:20022:tech:xsd:camt.056.001.09".to_string(),
    );
    let xml_declaration = r#"<?xml version="1.0" encoding="UTF-8"?>"#;
    let xml_body = xml_to_string(&envelope)?;
...
fn generate_camt057_xml(
    ...
    let header: bah_camt_057_001_06::BusinessApplicationHeaderV02 =
        deserialize(hd).map_err(|e| {
            ...
            let inner_err = e.into_inner().to_string();
            format!(
                "Failed to parse camt.057 header at path '{}': {}",
                path, inner_err
            )
        })?;
    ...
    let doc_str = serde_json::to_string(doc_content)?;
    let dd = &mut serde_json::Deserializer::from_str(&doc_str);
    let doc: camt_057_001_06::NotificationToReceiveV06 = deserialize(dd).map_err(|e| {
        let path = e.path().to_string();
        let inner_err = e.into_inner().to_string();
        error!(
            "Failed to parse camt.057 document at path '{}': {}",
            path, inner_err
        );
        format!(
            "Failed to parse camt.057 document at path '{}': {}",
    ...
    let envelope = MxEnvelope::new(
        header,
        doc,
        "urn:iso:std:iso:20022:tech:xsd:camt.057.001.06".to_string(),
    );
    let xml_declaration = r#"<?xml version="1.0" encoding="UTF-8"?>"#;
    let xml_body = xml_to_string(&envelope)?;
...
/// Generate camt.058 XML from parsed JSON
fn generate_camt058_xml(
    app_hdr: &Value,
    document: &Value,
) -> Result<String, Box<dyn std::error::Error>> {
    ...
    let header: bah_camt_058_001_08::BusinessApplicationHeaderV02 =
        deserialize(hd).map_err(|e| {
            ...
            let inner_err = e.into_inner().to_string();
            format!(
                "Failed to parse camt.058 header at path '{}': {}",
                path, inner_err
            )
        })?;
    ...
    let doc_str = serde_json::to_string(doc_content)?;
    let dd = &mut serde_json::Deserializer::from_str(&doc_str);
    let doc: camt_058_001_08::NotificationToReceiveCancellationAdviceV08 = deserialize(dd)
        .map_err(|e| {
            let path = e.path().to_string();
            let inner_err = e.into_inner().to_string();
            error!(
                "Failed to parse camt.058 document at path '{}': {}",
                path, inner_err
            );
            format!(
                "Failed to parse camt.058 document at path '{}': {}",
...
/// Generate camt.105 XML from parsed JSON
fn generate_camt105_xml(
    app_hdr: &Value,
    document: &Value,
) -> Result<String, Box<dyn std::error::Error>> {
    ...
    let header: bah_camt_105_001_02::BusinessApplicationHeaderV02 =
        deserialize(hd).map_err(|e| {
            ...
            let inner_err = e.into_inner().to_string();
            format!(
                "Failed to parse camt.105 header at path '{}': {}",
                path, inner_err
            )
        })?;
    ...
    let doc_content = document
        ...
        .ok_or("Missing ChrgsPaymtNtfctn in Document")?;

    debug!(
        "camt.105 doc_content: {}",
        serde_json::to_string_pretty(doc_content)?
    );
    let doc_str = serde_json::to_string(doc_content)?;
    let dd = &mut serde_json::Deserializer::from_str(&doc_str);
    let doc: camt_105_001_02::ChargesPaymentNotificationV02 = deserialize(dd).map_err(|e| {
        let path = e.path().to_string();
        let inner_err = e.into_inner().to_string();
        error!(
            "Failed to parse camt.105 document at path '{}': {}",
            path, inner_err
        );
        format!(
            "Failed to parse camt.105 document at path '{}': {}",
    ...
    let envelope = MxEnvelope::new(
        header,
        doc,
        "urn:iso:std:iso:20022:tech:xsd:camt.105.001.02".to_string(),
    );
    let xml_declaration = r#"<?xml version="1.0" encoding="UTF-8"?>"#;
    let xml_body = xml_to_string(&envelope)?;
...
fn generate_camt029_xml(
    ...
    let header: bah_camt_025_001_08::BusinessApplicationHeaderV02 =
        deserialize(hd).map_err(|e| {
            ...
            let inner_err = e.into_inner().to_string();
            format!(
                "Failed to parse camt.029 header at path '{}': {}",
                path, inner_err
            )
        })?;
    ...
    let doc_str = serde_json::to_string(doc_content)?;
    let dd = &mut serde_json::Deserializer::from_str(&doc_str);
    let doc: camt_029_001_09::ResolutionOfInvestigationV09 = deserialize(dd).map_err(|e| {
        let path = e.path().to_string();
        let inner_err = e.into_inner().to_string();
        error!(
            "Failed to parse camt.029 document at path '{}': {}",
            path, inner_err
        );
        format!(
            "Failed to parse camt.029 document at path '{}': {}",
...
fn generate_pain001_xml(
    ...
    let header: bah_pain_001_001_09::BusinessApplicationHeaderV02 =
        deserialize(hd).map_err(|e| {
            ...
            let inner_err = e.into_inner().to_string();
            format!(
                "Failed to parse pain.001 header at path '{}': {}",
                path, inner_err
            )
        })?;
    ...
    let doc_str = serde_json::to_string(doc_content)?;
    let dd = &mut serde_json::Deserializer::from_str(&doc_str);
    let doc: pain_001_001_09::CustomerCreditTransferInitiationV09 =
        deserialize(dd).map_err(|e| {
            let path = e.path().to_string();
            let inner_err = e.into_inner().to_string();
            error!(
                "Failed to parse pain.001 document at path '{}': {}",
                path, inner_err
            );
            format!(
                "Failed to parse pain.001 document at path '{}': {}",
...
fn generate_pain002_xml(
    ...
    let header: bah_pain_002_001_10::BusinessApplicationHeaderV02 =
        deserialize(hd).map_err(|e| {
            ...
            let inner_err = e.into_inner().to_string();
            format!(
                "Failed to parse pain.002 header at path '{}': {}",
                path, inner_err
            )
        })?;
    ...
    let doc_str = serde_json::to_string(doc_content)?;
    let dd = &mut serde_json::Deserializer::from_str(&doc_str);
    let doc: pain_002_001_10::CustomerPaymentStatusReportV10 = deserialize(dd).map_err(|e| {
        let path = e.path().to_string();
        let inner_err = e.into_inner().to_string();
        error!(
            "Failed to parse pain.002 document at path '{}': {}",
            path, inner_err
        );
        format!(
            "Failed to parse pain.002 document at path '{}': {}",
...
fn generate_pain008_xml(
    ...
    let header: bah_pain_008_001_08::BusinessApplicationHeaderV02 =
        deserialize(hd).map_err(|e| {
            ...
            let inner_err = e.into_inner().to_string();
            format!(
                "Failed to parse pain.008 header at path '{}': {}",
                path, inner_err
            )
        })?;
    ...
    let doc_str = serde_json::to_string(doc_content)?;
    let dd = &mut serde_json::Deserializer::from_str(&doc_str);
    let doc: pain_008_001_08::CustomerDirectDebitInitiationV08 = deserialize(dd).map_err(|e| {
        let path = e.path().to_string();
        let inner_err = e.into_inner().to_string();
        error!(
            "Failed to parse pain.008 document at path '{}': {}",
            path, inner_err
        );
        format!(
            "Failed to parse pain.008 document at path '{}': {}",
...
fn generate_pacs003_xml(
    ...
    let header: bah_pacs_003_001_08::BusinessApplicationHeaderV02 =
        deserialize(hd).map_err(|e| {
            ...
            let inner_err = e.into_inner().to_string();
            format!(
                "Failed to parse pacs.003 header at path '{}': {}",
                path, inner_err
            )
        })?;
    ...
    let doc_str = serde_json::to_string(doc_content)?;
    let dd = &mut serde_json::Deserializer::from_str(&doc_str);
    let doc: pacs_003_001_08::FIToFICustomerDirectDebitV08 = deserialize(dd).map_err(|e| {
        let path = e.path().to_string();
        let inner_err = e.into_inner().to_string();
        error!(
            "Failed to parse pacs.003 document at path '{}': {}",
            path, inner_err
        );
        format!(
            "Failed to parse pacs.003 document at path '{}': {}",
    ...
    let envelope = MxEnvelope::new(
        header,
        doc,
        "urn:iso:std:iso:20022:tech:xsd:pacs.003.001.08".to_string(),
    );
    let xml_declaration = r#"<?xml version="1.0" encoding="UTF-8"?>"#;
    let xml_body = xml_to_string(&envelope)?;
...
fn generate_admi024_xml(
    ...
    let header: bah_admi_024_001_01::BusinessApplicationHeaderV02 =
        deserialize(hd).map_err(|e| {
            ...
            let inner_err = e.into_inner().to_string();
            format!(
                "Failed to parse admi.024 header at path '{}': {}",
                path, inner_err
            )
        })?;
    ...
    let doc_str = serde_json::to_string(doc_content)?;
    let dd = &mut serde_json::Deserializer::from_str(&doc_str);
    let doc: admi_024_001_01::NotificationOfCorrespondenceV01 = deserialize(dd).map_err(|e| {
        let path = e.path().to_string();
        let inner_err = e.into_inner().to_string();
        error!(
            "Failed to parse admi.024 document at path '{}': {}",
            path, inner_err
        );
        format!(
            "Failed to parse admi.024 document at path '{}': {}",
    ...
    let envelope = MxEnvelope::new(
        header,
        doc,
        "urn:iso:std:iso:20022:tech:xsd:admi.024.001.01".to_string(),
    );
    let xml_declaration = r#"<?xml version="1.0" encoding="UTF-8"?>"#;
    let xml_body = xml_to_string(&envelope)?;
...
pub fn generate_mx_from_json(
    ...
    let envelope: GenericMxEnvelope = serde_json::from_value(json_data.clone()).map_err(|e| {
    ...
    let xml_string = match message_type {
        ...
        "camt.025" | "camt.060" => {
            return Err(Box::new(std::io::Error::other(format!(
                "XML generation for {} is not yet implemented",
                message_type
            ))));
        }
